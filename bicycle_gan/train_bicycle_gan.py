...
from generator_molkit import queue_datagen
...
if __name__ == '__main__':
    ...
    paths = []
    root = './data/refined-set'
    for subdir in os.listdir(root):
        if len(subdir) > 4:
            continue
        site_path = os.path.join(root, subdir, subdir + '_protein.pdb')
        lig_path = os.path.join(root, subdir, subdir + '_ligand.sdf')
        smi_path = os.path.join(root, subdir, subdir + '_ligand.smi')
        if not os.path.exists(smi_path):
            continue
        with open(smi_path, 'r') as f:
            lig_smi = f.read().split('\t')[0]

        paths.append([site_path, lig_path, lig_smi])
        # paths.append([site_path, lig_smi])
    ...
    plt_title = 'LiGANN Loss Curve during training (BATCH_SIZE=120)'
    plt.title(plt_title)
