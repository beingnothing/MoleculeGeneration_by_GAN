...
def shape_representation(file_pair):
    ...
    with open(pro_file, 'rb') as f:
        ...
        # if not pro_dic:
        #     return None
        protein_vox = pro_dic[smi_file.split('/')[3] + '_protein.pdb']
    try:
    ...
    if lig:
        try:
            smi_vox = voxelize(x=lig, format='smi')
        except:
            try:
...
def pros_representation(file_pair):
    ...
    try:
        ...
        try:
            lig = SmallMol(lig_file.split('.')[1] + '.mol2')
        except:
            return None
    try:
...
if __name__ == '__main__':
    ...
    for subdir in os.listdir(root):
        ...
        pro_dic = pros_representation([site_path, lig_path])
        if pro_dic:
            count += 1
