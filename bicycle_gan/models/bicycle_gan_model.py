...
# from models import networks_parallel as networks
...
class BiCycleGANModel(BaseModel):
    ...
    def encode(self, input_image):
        ...
        eps = self.get_z_random(std.size(0), std.size(1)).to(self.device)
    ...
    def forward(self,):
        half_size = len(self.real_A) // 2
        self.real_A_encoded = self.real_A[0:half_size]  # cLR image
        self.real_B_encoded = self.real_B[0:half_size]  # cVAE ground truth
        self.real_A_random = self.real_A[half_size:]
        self.real_B_random = self.real_B[half_size:]

        # get encoded z, self.z_encoded = cVAE Q(z|B)
        self.z_encoded, self.mu, self.logvar = self.encode(self.real_B_encoded)
        # get random z, self.z_random = cLR N_z
        self.z_random = self.get_z_random(self.real_A_encoded.size(0), self.opt.nz)
        # generate fake_B_encoded, self.fake_B_encoded = cVAE Network output
        self.fake_B_encoded = self.netG(self.real_A_encoded, self.z_encoded)
        # generate fake_B_random, self.fake_B_random = cLR Network output
        self.fake_B_random = self.netG(self.real_A_encoded, self.z_random)

        self.fake_data_encoded = torch.cat([self.real_A_encoded, self.fake_B_encoded], 1)  # cVAE
        self.real_data_encoded = torch.cat([self.real_A_encoded, self.real_B_encoded], 1)  # cVAE
        self.fake_data_random = torch.cat([self.real_A_encoded, self.fake_B_random], 1)  # cLR discriminator input
        self.real_data_random = torch.cat([self.real_A_random, self.real_B_random], 1)  #  cLR
        # compute z_predict
        if self.opt.lambda_z > 0.0:
            self.mu2, logvar2 = self.netE(self.fake_B_random)  # mu2 is a point estimate

        # self.real_A_encoded, real_B_encoded = x
        # self.cVAE_z_encoded, self.cVAE_mu, self.cVAE_logvar = self.encode(shapes)
        # self.cVAE_generted = self.generator(proteins, self.cVAE_z_encoded)
        #
        # self.cLR_generted = self.generator(proteins, N_z)
        # self.cLR_z_encoded, self.cLR_mu, self.cLR_logvar = self.encode(self.cLR_generted)
    ...
    def backward_G_alone(self):
        ...
        if self.opt.lambda_z > 0.0:
            self.loss_z_L1 = self.criterionZ(self.mu2.to(self.device), self.z_random.to(self.device)) * self.opt.lambda_z
