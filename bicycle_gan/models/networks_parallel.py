...
class D_N3Dv1LayersMulti(nn.Module):
    ...
    def get_layers(self, input_nc, ndf=64, norm_layer=nn.BatchNorm3d,
        ...
        kw = 3
        padw = 1
        sequence = nn.ModuleList([nn.Conv3d(input_nc, ndf, kernel_size=kw,
        ...
        sequence.extend(nn.ModuleList([nn.Conv3d(ndf * nf_mult, 1,
                               kernel_size=kw, stride=1, padding=padw)]))

        if use_sigmoid:
            sequence.extend(nn.ModuleList([nn.Sigmoid()]))
        return sequence
    ...
    def forward(self, input):
        ...
        for i in range(self.num_D):
            ...
            result.append(nn.parallel.data_parallel(self.model[i], down))
            if i != self.num_D - 1:
                # down = self.parallel_forward(self.down, down)
                down = nn.parallel.data_parallel(self.down, down)
        return result
...
class UnetBlock_with_z3D(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc, nz=0,
        ...
        downconv = nn.ModuleList([nn.Conv3d(input_nc, inner_nc,
        ...
        if outermost:
            ...
            down = downconv
            up = nn.ModuleList([uprelu]).extend(upconv).extend(nn.ModuleList([nn.Sigmoid()]))
        elif innermost:
            upconv = upsampleLayer(
                inner_nc, outer_nc, upsample=upsample, padding_type=padding_type, stride=stride)
            down = nn.ModuleList([downrelu]).extend(downconv)
            up = nn.ModuleList([uprelu]).extend(upconv)
            if norm_layer is not None:
                up += nn.ModuleList([norm_layer(outer_nc)])
        else:
            upconv = upsampleLayer(
                inner_nc * 2, outer_nc, upsample=upsample, padding_type=padding_type, stride=stride)
            down = nn.ModuleList([downrelu]).extend(downconv)
            if norm_layer is not None:
                down += nn.ModuleList([norm_layer(inner_nc)])
            up = nn.ModuleList([uprelu]).extend(upconv)
            ...
            if use_dropout:
                up += nn.ModuleList([nn.Dropout(0.5)])
...
def upsampleLayer(inplanes, outplanes, upsample='basic', padding_type='zero', stride=2):
    if upsample == 'basic':
        upconv = nn.ModuleList([nn.ConvTranspose3d(
...
def fetch_simple_block3d(in_lay, out_lay, nl, norm_layer, stride=1, kw=3, padw=1):
    return nn.ModuleList([nn.Conv3d(in_lay, out_lay, kernel_size=kw,
